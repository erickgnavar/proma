...
class InvoiceCreateViewTestCase(TestCase):
    ...
    def test_create_invoice(self):
        ...
        self.assertEqual(
            response["location"],
            reverse("invoices:invoice-detail", kwargs={"id": last_invoice.id}),
        )
...
class InvoiceUpdateViewTestCase(TestCase):
    def setUp(self):
        ...
        self.invoice = mixer.blend(
            "invoices.Invoice", client=self.client, project=self.project
        )
...
class InvoiceActionPayViewtTestCase(TestCase):
    ...
    def test_pay_successful(self):
        request = self.factory.post("/", {"payment_notes": "payment notes"})
        request.user = self.user
        request.session = {}
        request._messages = FallbackStorage(request)
        self.assertEqual(self.invoice.status, Invoice.OPEN)
